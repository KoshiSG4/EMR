...
const PatientsPage = () => {
	...
	const patientsData = patients.map((patient: Patient) => {
		return {
			userId: patient.userId ?? '',
			name: patient.fullName ?? '',
			emergencyContact: patient.emergencyContact ?? '',
			gender: patient.user.gender ?? '',
			age: calculateAge(new Date(patient.user.dateOfBirth)) ?? '',
			dateOfBirth: patient.user.dateOfBirth ?? '',
			email: patient.user.email ?? '',
			contactNo: patient.user.phone ?? '',
		};
	});

	const handleSelectPatient = (rowData: (typeof patientsData)[number]) => {
		const selectedPatient = patients.find(
			(patient: Patient) => patient.userId === rowData?.userId
		);
		if (selectedPatient) {
			...
			dispatch(
				getSelectedPatientData({ patientId: selectedPatient.userId })
			);
